    ...
    <div className={`relative z-10 max-w-4xl text-center space-y-6 transition-all duration-700 transform ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
      ...
      <h1 className="text-4xl md:text-6xl lg:text-7xl font-medium tracking-tighter text-balance text-foreground">
        <span className="italic font-bold text-transparent bg-clip-text bg-gradient-to-r from-orange-500 via-yellow-500 to-red-500">
          Too Long Didn't Read?<span className="ml-1">&nbsp;</span>
        </span>
        <span className="text-primary">Simplify Clauses</span> in Seconds!
      </h1>


      <p className="text-lg md:text-xl text-muted-foreground max-w-2xl mx-auto text-balance">
        Transform complex insurance documents into clear, understandable summaries. Upload your policy, get instant analysis, and never miss important clauses again.
      </p>
      ...
      <div className="flex flex-col sm:flex-row gap-4 justify-center pt-6 items-center">
        ...
        >
          Upload Your Document and Get Started
        </Button>
      ...
      <div className="pt-6 text-sm text-muted-foreground">
        No sign-up required • Instant analysis • Secure processing
      </div>
    </div>
